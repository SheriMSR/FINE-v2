...
public class ApiResponses<T> : ApiResponse
{
    ...
    public static ApiResponses<T> SuccessNotify(IEnumerable<T> data, int? totalCount = null, int? pageSize = null,
        int? offset = null,
        int? totalPages = null,
        int? totalUnread = null)
    {
        return new ApiResponses<T>
        {
            Data = data,
            ...
            PageSize = pageSize,
            Offset = offset,
            TotalPages = totalCount.HasValue && pageSize.HasValue
                ? (int)Math.Ceiling(totalCount.Value / (double)pageSize.Value)
                : totalPages,
        };
    }
...
public class ApiResponse
{
    ...
    private static ApiResponse Created(StatusCode statusCode, string message)
    {
